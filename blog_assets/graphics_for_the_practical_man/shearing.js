var sharing_demo_x_basis = {x: 1, y: 0};
var sharing_demo_y_basis = {x: 0, y: 1};
...
var Module = (function() {
  ...
  function render(dt) {
    ...
    var amount = 0.75*Math.sin(2*angle);
    if (angle > 2*Math.PI) {
      angle -= 2*Math.PI;
    } else if (angle < Math.PI) {
      shearBasis.xBasis.y = amount;
    } else {
      shearBasis.yBasis.x = amount;
    }
    
    shearing_demo_x_basis = shearBasis.xBasis;
    shearing_demo_y_basis = shearBasis.yBasis;
    
    starWithBasis(shearBasis);
